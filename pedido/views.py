...
def finalizar_pedido(request, room_name='pedir'):
    if request.method == "GET":
        ...
        if len(request.session['carrinho']) > 0:
            ...
            lambda_func_pagamento = lambda x: 'Cartão' if x['meio_pagamento'] == '2' else 'Dinheiro' if x['meio_pagamento'] == '1' else 'Pix'

            if x['meio_pagamento'] == '3':  # Supondo que '3' representa Pix
                qr_code_pix = gerar_qr_code_pix(total)
            else:
                qr_code_pix = None
            ...
            pedido = Pedido(
                ...
                bairro=x['bairro'],
                telefone=x['telefone'],
                qr_code_pix=qr_code_pix  # Adicione este campo ao seu modelo Pedido
            )
            pedido.save()
